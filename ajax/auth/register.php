...
if(checkUserSession($db) !== True){
	if(!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["re_password"])){
		...
		$profilePicture = "https://account.ggs-network.de/assets/img/default.jpg"; //default
						...
						if(strpos($username, " ") === False){
							if($password === $re_password){
								...
								if(mysqli_num_rows($sql) < 1){
									...
									mysqli_query($db, "INSERT INTO user(id, firstName, lastName, profilePicture, email, username, password, session, admin,verified,joinned_time) VALUES (NULL, '$firstName', '$lastName ', '$profilePicture', '$email', '$username', '$password', '$userSession', 0, 0, '$dateTime')");
